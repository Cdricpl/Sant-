...
export const BIOMARKERS_CATALOG: BiomarkerCatalogEntry[] = [
  // Glucides
  {
    ...
    unit: "mg/dL",
    category: "Métabolisme glucidique",
    refMin: 70,
    refMax: 100,
    refRange: "70 - 100",
  },
  ...
  {
    key: "quicki",
    name: "Quicki (sensibilité insuline)",
    unit: "",
    category: "Métabolisme glucidique",
    refMin: 0.35,
    refRange: "> 0.35",
...
export function flagForValue(
  entry: BiomarkerCatalogEntry,
  value: number,
  sex?: "M" | "F",
): "low" | "high" | "normal" {
  const min =
    sex === "M" && entry.refMinM !== undefined
      ? entry.refMinM
      : sex === "F" && entry.refMinF !== undefined
        ? entry.refMinF
        : entry.refMin;
  const max =
    sex === "M" && entry.refMaxM !== undefined
      ? entry.refMaxM
      : sex === "F" && entry.refMaxF !== undefined
        ? entry.refMaxF
        : entry.refMax;
